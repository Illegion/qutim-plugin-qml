...
namespace QmlPopups
{
	...
	void Popup::setMessage ( const QString& title, const QString& body, qutim_sdk_0_2::TreeModelItem item )
	{
		m_title = title;
		m_body = body;
		m_item = item;
	...
	void Popup::appendMessage ( const QString& message )
	{
		if (!message.isEmpty()) {
			m_body += "<br />" + message;
                        m_body = m_body.remove("<br />"+ tr("is typing..."));
			updateMessage();
		}
	}
	...
	void Popup::updateMessage()
	{
		...
		m_notification_widget->setData(m_title,m_body,m_item);
